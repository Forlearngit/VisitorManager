...
CVisitor::CVisitor(CPaintManagerUI* pManager):m_pManager(pManager)
{
	m_pList = static_cast<CListUI*>(m_pManager->FindControl(_T("Visitors")));
	m_iPage = 1;
}
int CVisitor::m_iCurrentPage = 1;
CVisitor::~CVisitor()
{
	m_pManager = NULL;
	m_pList = NULL;
...
void CVisitor::ShowStatus()
{
	int num = m_pList->GetCount();
	CLabelUI* pShow1 = static_cast<CLabelUI*>(m_pManager->FindControl(_T("Visitor_CurrentPage")));
	CLabelUI* pSHow2 = static_cast<CLabelUI*>(m_pManager->FindControl(_T("Visitor_CurrentVisitor")));
	WCHAR buf1[128];
	wsprintf(buf1,L"%s%d%s%s%d%s",L"当前页面：第",m_iCurrentPage,L"页,",L"共",m_iPage,L"页");
	WCHAR buf2[128];
